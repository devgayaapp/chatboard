...
class StreamViewMultiActionAgent:
    ...
    def __init__(
            ...
            seed=None,
            pydantic_model=None,
            add_agent_actions=True,
            **kwargs           
        ):
        self.context = context
        
        self.tools = tools
        if add_agent_actions:
            self.tools += [ HandoffAction, FinishAction ]
        # self.tools = tools 
        if not name:
    ...
    async def next_step(self, state, prompt=None, step_index=None, input_postfix=None, system_postfix=None, tracer_run=None, retry=3, **kwargs): 
        for i in range(retry):
            try:
                ...
                #     **kwargs,
                # )
                async for msg in self.prompt._call_llm_stream(openai_messages, func=self.name, tracer_run=tracer_run, pydantic_model=self.pydantic_model, **kwargs):
                    yield msg
        ...
        with Tracer(
            ...
            for i in range(self.max_iterations):
                with Tracer(
                    ...
                    # if not issubclass(ha_chunk.__class__, AgentState):
