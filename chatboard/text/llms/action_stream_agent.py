...
from components.etl.chat_prompt import ChatPrompt, ChatResponse, ChatChunk, PromptChunkTypes, validate_input_variables
...
class HandoffAction(BaseModel):
    """if you need to get more information about what to do you can ask the user for more information."""
    message: str = Field(..., description="the message to send to the user with the handoff request. write what information is missing.")
...
class AgentAction(BaseModel):
    ...
    def to_dict(self):
        d = self.dict()
        d['ActionType'] = self.__class__.__name__
        return d
...
class AgentState(BaseModel):
    ...
    def _should_handoff(self):
        return type(self._actions[-1]) == HandoffAction
...
class AgentChunk(BaseModel):
    ...
    def to_dict(self):
        return {
            ...
            "prompt_chunk": self.prompt_chunk.to_dict() if self.prompt_chunk else None,
...
class ActionStreamAgent:
    ...
    def __init__(
        ...
        self.tools = tools + [ HandoffAction, FinishAction ]
        # self.tools = tools 
        if not name:
    ...
    async def next_step(self, state, prompt=None, step_index=None, input_postfix=None, system_postfix=None, tracer_run=None, retry=3, **kwargs): 
        for i in range(retry):
            try:
                ...
                #     **kwargs,
                # )
                async for msg in self.prompt._call_action_llm_stream(openai_messages, func=self.name, tracer_run=tracer_run, pydantic_model=self.pydantic_model, **kwargs):
                    if msg.msg_type == PromptChunkTypes.PROMPT_MILESTONE:
                        llm_tool_completion, tool_calls = await self.parse_tool(msg.content.Action_Input, tracer_run=tracer_run)
...
"""
        ...
        for action in tools[:self.max_actions]:            
            with tracer_run.create_child(
                ...
                if type(action) == FinishAction:
                    ...
                    yield state
                    return
                if type(action) == HandoffAction:
                    tool_run.end(outputs={
                        "state": state,
                        "action_output": "handoff",
                    })
                    state._actions.append(action)
                    yield state
                    return
                if type(action) in self.thunks:
                    async for chunk in await self.thunks[type(action)](self.context, action, tracer_run=tool_run): yield chunk
                    if isinstance(chunk, ChatResponse):
                        chunk = chunk.value
                    if not issubclass(chunk.__class__, AgentAction):
                        raise ValueError("thunk return value must be an AgentAction")
                    action = chunk
    ...
    async def run(self, state, tracer_run=None, **kwargs):
        async for msg in self.step_generator(state=state, tracer_run=tracer_run, **kwargs):
            # if type(msg) == AgentChunk:
            #     if msg.msg_type == AgentChunkTypes.AGENT_FINISH:
            #         break
            yield msg
        # yield msg
        ...
        with Tracer(
            ...
            for i in range(self.max_iterations):
                with Tracer(
                    ...
                    async for chunk in self.next_step(state, tracer_run=step_run, **kwargs):
                        if type(chunk) == ChatChunk:
                            yield chunk
                    state, tools = chunk

                    async for ha_chunk in self.handle_action(state, tools, tracer_run=step_run, **kwargs):
                        yield ha_chunk

                    if not isinstance(ha_chunk, AgentState):
                        raise ValueError("handle_action must return an AgentState")
                    state = ha_chunk
                    ...
                    if state._did_finish():
                        did_stop = True
                        break
                    if state._should_handoff():
                        should_handoff = True
                        break
                    yield AgentChunk(
                        msg_type=AgentChunkTypes.AGENT_FINISH,
                        func=state._actions[-1].__class__.__name__,
                        state=state,
                    ) 
